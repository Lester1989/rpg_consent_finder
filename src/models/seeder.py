...
def seed_playfun_questions():
    ...
    with Session(engine) as session:
        ...
        for play_style, statements in raw.items():
            logging.debug(f"Topic: {play_style}")
            for rating, statements in statements.items():
                for statement_de, statement_en in statements:
                    ...
                    if not existing_question:
                        local_statement = LocalizedText(
                            text_en=statement_en, text_de=statement_de
                        )
                        existing_text = any(
                            text.text_de == statement_de for text in existing_texts
                        )
                        if not existing_text:
                            session.add(local_statement)
                            session.commit()
                            session.refresh(local_statement)
                        session.add(
                            PlayFunQuestion(
