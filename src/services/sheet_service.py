...
def create_consent_sheet(
    ...
    *,
    ...
    def _create(active_session: Session) -> ConsentSheet:
        ...
        unique_name = name_factory(active_session)
        ...
        active_session.add(sheet)
        active_session.commit()
        active_session.refresh(sheet)

        templates = active_session.exec(select(ConsentTemplate)).all()
        ...
        active_session.commit()
        active_session.refresh(sheet)
        active_session.merge(managed_user)
        active_session.commit()
